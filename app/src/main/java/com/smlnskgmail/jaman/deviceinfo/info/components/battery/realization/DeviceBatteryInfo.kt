package com.smlnskgmail.jaman.deviceinfo.info.components.battery.realization
...
class DeviceBatteryInfo(private val context: Context) : BatteryInfo {
    ...
    private fun healthStatusFromIntent(status: Int): String {
        return when (status) {
            BatteryManager.BATTERY_HEALTH_COLD -> context.getString(R.string.battery_info_health_cold)
            ...
            else -> context.getString(R.string.battery_info_common_unknown)
        }
    }
    ...
    private fun pluggedStatusFromIntent(status: Int): String {
        return when (status) {
            BatteryManager.BATTERY_PLUGGED_WIRELESS -> context.getString(R.string.battery_info_plugged_wireless)
            BatteryManager.BATTERY_PLUGGED_USB -> context.getString(R.string.battery_info_plugged_usb)
            BatteryManager.BATTERY_PLUGGED_AC -> context.getString(R.string.battery_info_plugged_ac)
            else -> context.getString(R.string.battery_info_common_unknown)
        }
    }
    ...
    private fun actionStatusFromIntent(status: Int): String {
        return when (status) {
            BatteryManager.BATTERY_STATUS_CHARGING -> context.getString(R.string.battery_info_action_charging)
            BatteryManager.BATTERY_STATUS_FULL -> context.getString(R.string.battery_info_action_full)
